...
#include <strings.h>  /* for bzero  - jea*/
...
static int Hash_setattr(DOH *ho, DOH *k, DOH *obj) {
...
#if 0
  /* jea debug  */
  /*Printf(stdout," Hash_setattr, setting key:value    '%s':'%s'\n",Char(k),Char(obj)); */
  FILE* kv;
  kv = fopen("/home/jaten/dj/kvcheck.out","w+");
  ...
  fclose(kv);
  /*  printf("debug:read '%s':'%s' from key:value k:obj\n",kbuf,vbuf); */

  if (0==(strcmp(kbuf,"D"))) {
    printf("stopping on Key 'D'\n");
    sync();
  ...
  if (0==(strcmp(kbuf,"newname"))) {
    printf("stopping on Key 'newname'\n");
    sync();
    printf("set breakpoint here\n");
    Printf(stdout," Hash_setattr, setting key:value    '%s':'%s'\n",Char(k),Char(obj)); 
    sync();
  }
  if (0==(strcmp(vbuf,"newname"))) {
    printf("stopping on Value 'newname'\n");
    sync();
    printf("set breakpoint here\n");
    Printf(stdout," Hash_setattr, setting key:value    '%s':'%s'\n",Char(k),Char(obj)); 
    sync();
  ...
  if (0==(strcmp(kbuf,"rename_me"))) {
    printf("stopping on Key 'rename_me'\n");
    sync();
    printf("set breakpoint here\n");
    Printf(stdout," Hash_setattr, setting key:value    '%s':'%s'\n",Char(k),Char(obj)); 
    sync();
  }
  if (0==(strcmp(vbuf,"rename_me"))) {
    printf("stopping on Value 'rename_me'\n");
    sync();
    printf("set breakpoint here\n");
    Printf(stdout," Hash_setattr, setting key:value    '%s':'%s'\n",Char(k),Char(obj)); 
    sync();
